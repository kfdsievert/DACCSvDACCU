...
# Overlay custom boxplot (without whiskers) for SAF (y=1)
box_height = 0.08
q25_saf = 424
q75_saf = 1838
q50_saf = 1573

# Draw filled rectangle for IQR
rect_saf = plt.Rectangle(
    (q25_saf, 1 - box_height / 2),
    q75_saf - q25_saf,
    box_height,
    edgecolor="#444343",         # dark gray edge
    facecolor="#444343",         # dark gray fill
    linewidth=1.5,
    zorder=3
)
ax_violin.add_patch(rect_saf)

# Draw white median line that covers the full box height
ax_violin.plot(
    [q50_saf, q50_saf],
    [1 - box_height / 2, 1 + box_height / 2],
    color='white',
    linewidth=2.5,
    zorder=4
)

# Overlay custom boxplot (without whiskers) for DACCS (y=0)
q25_daccs = 160
q75_daccs = 542
q50_daccs = 312

rect_daccs = plt.Rectangle(
    (q25_daccs, 0 - box_height / 2),
    q75_daccs - q25_daccs,
    box_height,
    edgecolor='#444343',         # dark gray edge
    facecolor="#444343",         # dark gray fill
...
ax_violin.add_patch(rect_daccs)

# Draw white median line that covers the full box height
ax_violin.plot(
    [q50_daccs, q50_daccs],
    [0 - box_height / 2, 0 + box_height / 2],
    color='white',
    linewidth=2.5,
    zorder=4
)
